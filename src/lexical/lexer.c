...
void	lexer(t_cmd **cmds, char **input, t_env **env, int *error)
{
	...
	if (valid_syntax1(input, error)) // check or leading | or "&&", "||" .
		return ;
	tokenizer(&tokens, *input, error); // generate array of tokens
		+ validate syntax2 .
