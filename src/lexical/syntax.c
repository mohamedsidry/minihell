...
/*   Updated: 2025/09/20 19:15:55 by msidry           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../include/main.h"

static int leading_pipe(char *input);
...
int valid_syntax(char **input, int *error)
{
    ...
    if (!input || !(*input))
        return (0);
    tmp = *input;
    if (bonus_syntax(tmp) || leading_pipe(tmp))
    {
        free(*input);
        *input = NULL;
        *error = 258;
...
int valid_syntax2(char **token, int *error)
{
    ...
    while (token[idx])
    {
        if (!ft_strncmp(token[idx], ">>", 3) 
        || !ft_strncmp(token[idx], "<<", 3)
        || !ft_strncmp(token[idx], ">", 2)
        )
        {
            if (!token[idx + 1])
                return (*error = 258, syntax_error("newline"));
            else if (is_redirection(token[idx + 1]))
